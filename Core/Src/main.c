...
int main(void)
{
  ...
  osThreadDef(ADC, ADCRead, osPriorityNormal, 0, 128);
...
void ADCRead(void const * argument)
{
  ...
  for(;;)
  {
    ...
    if (HAL_ADC_PollForConversion(&hadc1, 10) == HAL_OK){
    	...
    	//adcValue= voltage*4096.0f/3.3f
    	//0,5*4096.0f/3.3f = 620
    	if(adcValue > 620){
    ...
    HAL_ADC_Stop(&hadc1); // Останавливаем АЦП до следующего раза
    vTaskDelay(pdMS_TO_TICKS(50)); // Засыпаем на 50 мс (отдаем процессор ОС)
  }
  /* USER CODE END 5 */
}
